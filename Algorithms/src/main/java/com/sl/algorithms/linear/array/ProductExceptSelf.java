...
public class ProductExceptSelf {
     ...
     */
    public static Integer[] productExceptSelf(Integer[] nums) {
        if (nums == null || nums.length == 0) {
            return nums;
        }
        int length=nums.length, left=1, right=1;
        Integer[] output = new Integer[length]; // not counted towards space as this is not used for any temp storage
        output[0] = 1;
