 ...
 * <br>Time : O(N) average case, O(N^2) worst case
 * <br>Space: O(1)
 */
...
public interface QuickSelect<T extends Comparable> extends BaseInterface<T> {
    ...
    default void kCheck(int n, int k) {
        if (k < 1) throw new IllegalArgumentException("kMax must be at least 1");
        if (n == 1 && k != 1) throw new IllegalArgumentException("kMax can only be 1 for a single-element array");
        if (n > 1 && k > n) throw new IllegalArgumentException("kMax is higher than the highest index");
    }
}
