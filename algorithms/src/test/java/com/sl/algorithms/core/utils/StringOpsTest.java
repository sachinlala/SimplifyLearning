...
public class StringOpsTest extends StringOps {
    ...
    public void assertFindPermutations() {
        ...
        { // lower case
            ...
            Assert.assertEquals("[ABC, aBC, AbC, abC, ABc, aBc, Abc, abc]", findPermutations("abc").toString());
            Assert.assertEquals("[A1B2C, a1B2C, A1b2C, a1b2C, A1B2c, a1B2c, A1b2c, a1b2c]", findPermutations("a1b2c").toString());
            Assert.assertEquals("[ABCD, aBCD, AbCD, abCD, ABcD, aBcD, AbcD, abcD, ABCd, aBCd, AbCd, abCd, ABcd, aBcd, Abcd, abcd]", findPermutations("abcd").toString());
        }
        { // upper case
            Assert.assertEquals("[A1, a1]", findPermutations("A1").toString());
            Assert.assertEquals("[AB, aB, Ab, ab]", findPermutations("AB").toString());
